...
const Tokens: React.FC<Props> = ({ isVisible, onClose, selectedCompany }) => {
  ...
  return isVisible ? (
    <Styled.Background>
      <Styled.Wrapper>
        <Styled.Header>
          <Styled.Title>{t('COMPANY_SCREEN.TOKENS')}</Styled.Title>

          <Styled.Close name="close" size="24px" onClick={onClose} />
        </Styled.Header>

        <Button
          text={t('COMPANY_SCREEN.ADD_TOKEN')}
          rounded
          width={150}
          icon="plus"
        ...
        <Styled.Table>
          ...
          <Styled.Head>
            <Styled.Column>{t('COMPANY_SCREEN.TOKEN')}</Styled.Column>
            <Styled.Column>{t('COMPANY_SCREEN.DESCRIPTION')}</Styled.Column>
            <Styled.Column>{t('COMPANY_SCREEN.EXPIRES')}</Styled.Column>
            <Styled.Column>{t('COMPANY_SCREEN.ACTION')}</Styled.Column>
          </Styled.Head>

          <Styled.Body>
            {!tokens || tokens.length <= 0 ? (
              <Styled.EmptyText>
                {t('COMPANY_SCREEN.NO_TOKENS')}
              </Styled.EmptyText>
            ) : null}
      ...
      <Dialog
        message={t('COMPANY_SCREEN.CONFIRM_DELETE_TOKEN')}
