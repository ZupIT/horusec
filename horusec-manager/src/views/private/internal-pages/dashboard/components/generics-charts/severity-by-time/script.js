...
export default {
  ...
  mounted () {
    this.$eventBus.$on('udpate-date', (event) => {
      ...
      if (!this.getterLoadingChartSeverityByTime) {
        this.getDataChart()
      }
    })
  },
  ...
  methods: {
    ...
    getDataChart () {
      ...
      return this.actionGetChartSeverityByTime(params)
        .then((result) => {
          if (result.data.content && result.data.content.length) {
            result.data.content.forEach((element) => {
              this.chart.labels = this.chart.labels.concat(element.time)
