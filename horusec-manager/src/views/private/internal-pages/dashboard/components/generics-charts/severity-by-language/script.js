...
export default {
  ...
  mounted () {
    this.$eventBus.$on('udpate-date', (event) => {
      ...
      if (!this.getterLoadingChartSeverityByLanguage) {
        this.getDataChart()
      }
    })
  },
  ...
  methods: {
    ...mapActions([
      'actionGetChartSeverityByLanguage'
    ]),
    getDataChart () {
      const params = {
      ...
      this.actionGetChartSeverityByLanguage(params)
        .then((result) => {
          if (result.data.content && result.data.content.length) {
            result.data.content.forEach((element) => {
              this.chart.labels = this.chart.labels.concat(element.language)
