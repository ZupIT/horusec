...
export default {
  ...
  mounted () {
    this.$eventBus.$on('udpate-date', (event) => {
      ...
      if (!this.getterLoadingChartSeverityByRepository) {
        this.getDataChart()
      }
    })
  },
  ...
  methods: {
    ...mapActions([
      'actionGetChartSeverityByRepository'
    ]),
    getDataChart () {
      const params = {
      ...
      this.actionGetChartSeverityByRepository(params)
        .then((result) => {
          if (result.data.content && result.data.content.length) {
            result.data.content.forEach((element) => {
              this.chart.labels = this.chart.labels.concat(element.repository)
