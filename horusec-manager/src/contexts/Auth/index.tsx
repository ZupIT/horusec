...
const MANAGER_PATH: string = (window as any).REACT_APP_HORUSEC_MANAGER_PATH || '';
      ...
      return new Promise((resolve) => {
        getAuthenticator()
          ...
          .finally(() => {
            resolve();
            clearLocalStorage();
            window.location.replace(`${MANAGER_PATH.endsWith('/') ? MANAGER_PATH.slice(0, -1) : MANAGER_PATH }/auth`);
          });
      });
    } else {
      clearLocalStorage();
