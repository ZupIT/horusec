...
cloneInternalInTmpFolder () {
    ...
    then
        ...
        rm -rf "./$HORUSEC_INTERNAL_FOLDER/.git"
    fi
}

copyContentFromTmpToOpenSource () {
    echo "Coping all content of internal to internal..."
    cd "./$HORUSEC_INTERNAL_FOLDER"
    rsync -a -r -v ./ ../
    if [ $? != "0" ]
    then
        echo "ERROR on copy internal to internal content!"
        exit 1
    fi
    cd ..
}
...
removeTmpFolder () {
    ...
    if [ $? != "0" ]
    then
        echo "ERROR on remove internal folder!"
        exit 1
    fi
}
