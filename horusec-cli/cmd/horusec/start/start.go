...
func (s *Start) loadFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().
		StringVarP(&s.configs.ToolsToIgnore, "tools-ignore", "T", s.configs.GetToolsToIgnore(),
			"Tools to ignore in the analysis. Available are: GoSec,SecurityCodeScan,Brakeman,Safety,Bandit,NpmAudit,YarnAudit,HorusecKotlin,HorusecJava,HorusecLeaks,GitLeaks,TfSec,Semgrep,HorusecCsharp,Eslint. Example: -T=\"GoSec, Brakeman\"")
}
