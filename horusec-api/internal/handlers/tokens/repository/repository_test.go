...
func TestPost(t *testing.T) {
	...
	t.Run("should return status 200 when successfully create a token", func(t *testing.T) {
		...
		url := fmt.Sprintf("api/companies/%s/repositories/%s/tokens", token.CompanyID.String(), token.RepositoryID.String())
		r, _ := http.NewRequest(http.MethodPost, url, bytes.NewReader(token.ToBytes()))
		w := httptest.NewRecorder()

		ctx := chi.NewRouteContext()
		ctx.URLParams.Add("repositoryID", token.RepositoryID.String())
	...
	t.Run("should return status 422 when repositoryID is not valid", func(t *testing.T) {
		token := &api.Token{
			CompanyID:   uuid.New(),
			Description: "text",
		}
		...
		url := fmt.Sprintf("api/companies/%s/repositories/some_text/tokens", token.CompanyID.String())
		r, _ := http.NewRequest(http.MethodPost, url, bytes.NewReader(token.ToBytes()))
		w := httptest.NewRecorder()

		ctx := chi.NewRouteContext()
		ctx.URLParams.Add("repositoryID", "some_text")
		ctx.URLParams.Add("companyID", token.CompanyID.String())
	...
	t.Run("should return status 422 when companyId is not valid", func(t *testing.T) {
		...
		url := fmt.Sprintf("api/companies/some_text/repositories/%s/tokens", token.RepositoryID.String())
		r, _ := http.NewRequest(http.MethodPost, url, bytes.NewReader(token.ToBytes()))
		w := httptest.NewRecorder()
...
func TestDelete(t *testing.T) {
	...
	t.Run("should return status 204 when successfully delete a token", func(t *testing.T) {
		...
		url := fmt.Sprintf("api/companies/%s/repositories/%s/tokens/%s", token.CompanyID.String(), token.RepositoryID.String(), token.TokenID.String())
	...
	t.Run("should return status 400 when parse tokenID fails", func(t *testing.T) {
		...
		url := fmt.Sprintf("api/companies/%s/repositories/%s/tokens/invaliduuidstring", token.CompanyID.String(), token.RepositoryID.String())
		r, _ := http.NewRequest(http.MethodDelete, url, nil)
		w := httptest.NewRecorder()
	...
	t.Run("should return status 400 when tokenID is nil", func(t *testing.T) {
		...
		url := fmt.Sprintf("api/companies/%s/repositories/%s/tokens/%s", token.CompanyID.String(), token.RepositoryID.String(), uuid.Nil.String())
		r, _ := http.NewRequest(http.MethodDelete, url, nil)
		w := httptest.NewRecorder()
	...
	t.Run("should return status 404 when not exist token", func(t *testing.T) {
		...
		url := fmt.Sprintf("api/companies/%s/repositories/%s/tokens/%s", token.CompanyID.String(), token.RepositoryID.String(), token.TokenID)
		r, _ := http.NewRequest(http.MethodDelete, url, nil)
		w := httptest.NewRecorder()
...
func TestGetAllOfAccount(t *testing.T) {
	...
	t.Run("should return status 200 when successfully create a token", func(t *testing.T) {
		...
		url := fmt.Sprintf("api/companies/%s/repositories/%s/tokens", companyID.String(), repositoryID.String())
		r, _ := http.NewRequest(http.MethodGet, url, nil)
		w := httptest.NewRecorder()

		ctx := chi.NewRouteContext()
		ctx.URLParams.Add("repositoryID", companyID.String())
		ctx.URLParams.Add("companyID", repositoryID.String())
	...
	t.Run("should return status 400 when repositoryID is invalid", func(t *testing.T) {
		repositoryID := uuid.New()
		companyID := uuid.New()
		tokens := &[]api.Token{
			{
		...
		url := fmt.Sprintf("api/companies/%s/repositories/some_text/tokens", companyID.String())
		r, _ := http.NewRequest(http.MethodGet, url, nil)
		w := httptest.NewRecorder()
